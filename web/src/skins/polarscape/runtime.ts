...
import { PetActor, POLARSCAPE_PET_CONFIG } from '../pets/actor'

class PolarscapeRuntime implements WorldSkinRuntime {
  private application: Application | null = null
  private readonly pet = new PetActor(POLARSCAPE_PET_CONFIG)
  ...
  async mount(container: HTMLElement, context: WorldSkinRuntimeContext): Promise<void> {
    ...
    try {
      ...
      const packageUrl = new URL(POLARSCAPE_PET_CONFIG.packagePath, document.baseURI)
...
export const createWorldSkinRuntime = (): WorldSkinRuntime => new PolarscapeRuntime()
